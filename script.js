...
const CartManager = {
    ...
    addToCart: (productId, quantity = 1) => {
        // Ensure we always work with a numeric product id
        const numericId = Number(productId);
        const product = SKYRA.products.find(p => p.id === numericId);
        if (!product) return false;
        
        const existingItem = SKYRA.cart.find(item => item.product.id === numericId);
    ...
    updateCartUI: () => {
        ...
        // If cart UI elements are not on this page (e.g. wishlist.html),
        // stop after updating the header bubble.
        if (!cartItems || !cartEmpty || !cartFooter || !subtotalElement || !shippingElement || !totalElement) {
            return;
        }
...
const WishlistManager = {
        ...
        document.querySelectorAll('.wishlist-btn').forEach(btn => {
            ...
            if (isInWishlist) {
                ...
                if (icon) {
                    ...
                    icon.style.fill = 'var(--color-primary-700)';
                }
            } else {
    ...
    updateWishlistCount: () => {
        ...
        if (countEl) {
            countEl.textContent = SKYRA.wishlist.length;
            countEl.style.display = SKYRA.wishlist.length > 0 ? 'flex' : 'none';
        }
    }
};
...
const NavigationManager = {
    // Handle navigation
    init: () => {
        // Desktop navigation (only hijack in-page hash links)
...
const SearchManager = {
    init: () => {
        ...
        // Clicking the icon toggles the header search field
        if (searchBtn) {
            searchBtn.addEventListener('click', (e) => {
                e.preventDefault();

                // If header search exists, toggle it; otherwise fall back to prompt
                if (headerSearchForm && headerSearchInput) {
                    headerSearchForm.classList.toggle('active');
                    if (headerSearchForm.classList.contains('active')) {
                        headerSearchInput.focus();
                    } else {
                        // If closed, reset results to all products
                        SearchManager.performSearch('');
                    }
                } else {
                    SearchManager.toggleSearchPrompt();
                }
            });
        }

        // Header search form behaviour
        if (headerSearchForm && headerSearchInput) {
            headerSearchForm.addEventListener('submit', (e) => {
                e.preventDefault();
                SearchManager.performSearch(headerSearchInput.value);
            });
    ...
    // Legacy prompt-based search (kept for the header icon click)
    toggleSearchPrompt: () => {
    ...
    // Shared search implementation used by both prompt and inline search bar
    performSearch: (query) => {
        ...
        if (!trimmedQuery) {
            // Empty query: show all products again
            ProductManager.renderProducts(container, SKYRA.products);
            Utils.scrollToElement('#shop');
            return;
        }
    ...
    init: () => {
        ...
        if (contactForm) {
            contactForm.addEventListener('submit', (e) => {
                ...
                Utils.showToast('Thank you for your message! We\'ll get back to you within 24 hours.', 'success');
                contactForm.reset();
            });
        }
    }
